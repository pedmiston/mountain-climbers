...
(gg_single_dimension <- ggplot(ability) +
  aes(vision, vision, label = vision) +
  geom_bar(aes(alpha = vision), stat = "identity", fill = theme_colors[["blue"]]) +
  geom_text(vjust = -0.5) +
  scale_x_continuous("", breaks = NULL) +
  scale_y_continuous("vision", breaks = 1:10, labels = NULL,
                     position = "right") +
  scale_alpha_continuous(range = c(0.5, 1.0)) +
  guides(alpha = "none") +
  coord_cartesian(xlim = c(1, 10), ylim = c(0, 9.6)) +
  base_theme +
  theme(
...
```{r}
# Calculate the range of positions that could be search by this team
calculate_search_area <- function(vision_x, vision_y) {
  # vision of 1 gives a visible range of 3 (-1, 0, 1)
  visible_range <- function(vision) vision * 2 + 1
  visible_range(vision_x) * visible_range(vision_y)
}
...
diachronic_team_search_areas <- search_areas %>% mutate(strategy = "diachronic")
